...
function TaskNumbers({ data }) {
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-10'>
      ...
      {/* Failed Tasks Card */}
      <div className='relative rounded-2xl bg-gradient-to-br from-red-500 to-orange-600 p-6 transition-all duration-300 hover:scale-105 hover:shadow-xl'>
        <div className='flex items-center justify-between'>
        ...
        <h2 className='text-5xl font-bold text-white mt-4 mb-2'>{data?.taskCount?.failed}</h2>
        <p className='text-lg font-medium text-orange-100'>Failed Tasks</p>
        <div className='absolute bottom-0 left-0 right-0 h-2 bg-black/10 rounded-b-2xl' />
      </div>
    </div>
  );
